...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """设置图谱本体（公开方法）。具体动态类构造由后端负责。"""
        self.memory.set_ontology(graph_id, ontology)
    ...
    def add_text_batches(
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            # 构建episode数据并发送到后端
            episodes_payload = [{"data": chunk, "type": "text"} for chunk in batch_chunks]

            try:
                batch_result = self.memory.add_batch(
                    graph_id=graph_id,
    ...
    def _wait_for_episodes(
        ...
        while pending_episodes:
            ...
            for ep_uuid in list(pending_episodes):
                try:
                    if self.memory.get_episode_processed(ep_uuid):
    ...
    def _get_graph_info(self, graph_id: str) -> GraphInfo:
        """获取图谱信息"""
        # 获取节点（分页）
        nodes = fetch_all_nodes(self.memory, graph_id)

        # 获取边（分页）
        edges = fetch_all_edges(self.memory, graph_id)
